...
# -----------------------------
# Content generation (seeded, session-unique)
# -----------------------------
...
def gen_numeric_question(seed: int, box_id: int, player_count: int) -> Tuple[str, int]:
    ...
    k = rng.randint(3, 11)
    # Build answer to match template in a deterministic way.
    # We'll parse the chosen template style by checking markers.
    if "seed % 97" in tpl:
...
def gen_order_question(seed: int, box_id: int) -> Tuple[str, List[str], List[int]]:
    ...
    items = []
    values = []
    # Create five unique synthetic items; ordering by numeric value.
    for i in range(5):
        v = rng.randint(10, 99)
        while v in values:
            v = rng.randint(10, 99)
    ...
    # Determine correct order based on mode
    if "earliest to latest" in mode or "first to last" in mode:
        # smaller value = earlier/first
        sorted_pairs = sorted(list(enumerate(values)), key=lambda x: x[1])
    else:
        sorted_pairs = sorted(list(enumerate(values)), key=lambda x: x[1])
    correct_indices = [idx for idx, _ in sorted_pairs]  # 0..4
...
def gen_phrase_and_deck(seed: int, box_id: int) -> Tuple[Tuple[str, str, str], List[dict]]:
    ...
    # Card rules by box:
    # Box 1: PIECE + PASS only
    # Box 2-5: PIECE + PASS + STEAL (no DONATE/WILDCARD)
    # Box 6: PIECE + PASS + STEAL + DONATE + WILDCARD
    deck: List[dict] = []

    # Ensure at least 3 PIECE cards exist revealing each word at least once.
    deck.append({"type": "PIECE", "reveal": "W1"})
    deck.append({"type": "PIECE", "reveal": "W2"})
    deck.append({"type": "PIECE", "reveal": "W3"})

    # Fill remaining 7 cards according to box constraints.
    remaining = 7
    if box_id == 1:
        # More pieces + some pass
        for _ in range(remaining):
            deck.append({"type": rng.choices(["PIECE", "PASS"], weights=[7, 3])[0], "reveal": rng.choice(["W1", "W2", "W3"])})
        # Normalize PASS cards to omit reveal field
        for c in deck:
            if c["type"] == "PASS":
                c.pop("reveal", None)
...
class PuzzleModal(discord.ui.Modal, title="Submit Puzzle"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        if isinstance(channel, discord.TextChannel):
            embed = discord.Embed(title=f"Puzzle Attempt #{attempt_id}", description=f"Player: {user.mention}\nGuess: **{g1} {g2} {g3}**\nStatus: Pending host check")
...
class CardPickView(discord.ui.View):
    ...
    def _build_buttons(self):
        # Build 10 buttons (or fewer if deck smaller)
        # Button label shows Card 1..10 and disabled if revealed
        con = db()
        try:
            row = con.execute(
...
class CardButton(discord.ui.Button):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        con = db()
        try:
            # Verify slot holder and turns_left
            slot = con.execute(
                "SELECT slot_user_id, turns_left, pending_action FROM slot_state WHERE guild_id=? AND channel_id=? AND box_id=(SELECT current_box FROM sessions WHERE guild_id=? AND channel_id=?)",
            ...
            sess = con.execute(
                "SELECT current_box FROM sessions WHERE guild_id=? AND channel_id=?",
                (guild_id, channel_id),
            ).fetchone()
            ...
            # Mark revealed and decrement turns
            ...
            if card["type"] == "PIECE":
                ...
                pending_action = "PASS"
            elif card["type"] == "STEAL":
                pending_action = "STEAL"
            elif card["type"] == "DONATE":
                pending_action = "DONATE"
            elif card["type"] == "WILDCARD":
                # Random effect: PASS/STEAL/DONATE/BONUS_TURN (donate only if box 6)
                # BONUS_TURN: +1 turn (cap 5)
        ...
        # Disable clicked button and refresh view
        try:
...
class SubmitPuzzleView(discord.ui.View):
    ...
    async def submit_puzzle(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_modal(PuzzleModal(self.bot, self.guild_id, self.channel_id, self.box_id))
...
def get_current_box(con: sqlite3.Connection, guild_id: int, channel_id: int) -> int:
    row = con.execute("SELECT current_box FROM sessions WHERE guild_id=? AND channel_id=?", (guild_id, channel_id)).fetchone()
    return int(row["current_box"]) if row else 1
...
def next_closest_puzzle_attempt(con: sqlite3.Connection, guild_id: int, channel_id: int, box_id: int, correct_words: Tuple[str,str,str], exclude_attempt_id: int) -> Optional[sqlite3.Row]:
    # Choose among unchecked attempts excluding the one just checked.
    ...
    # "closest" = highest position matches; tie = fastest
    scored.sort(key=lambda t: (-t[0], t[1]))
    return scored[0][2]
...
async def lock(interaction: discord.Interaction):
    ...
    try:
        ...
        # ensure box 1 secret/deck exists
...
@wib.command(name="q_numeric", description="Generate a numeric question preview for current box (host preview with Publish/Regenerate).")
async def q_numeric(interaction: discord.Interaction):
    if not isinstance(interaction.user, discord.Member) or not has_host_role(interaction.user):
        return await interaction.response.send_message("Host permission required.", ephemeral=True)
...
@wib.command(name="num", description="Submit your numeric answer for the active question.")
@app_commands.describe(value="Your numeric answer")
async def num(interaction: discord.Interaction, value: int):
    con = db()
    try:
        sess = con.execute("SELECT * FROM sessions WHERE guild_id=? AND channel_id=?", (interaction.guild_id, interaction.channel_id)).fetchone()
        if not sess or sess["is_locked"] != 1:
...
@wib.command(name="reveal", description="Host: reveal numeric winner (exact->fastest, else closest->fastest) and assign slot.")
async def reveal(interaction: discord.Interaction):
    ...
    try:
        ...
        box_id = int(sess["current_box"])
        tr = con.execute(
            "SELECT q_text, answer_int, is_active FROM trivia_rounds WHERE guild_id=? AND channel_id=? AND box_id=?",
            (interaction.guild_id, interaction.channel_id, box_id),
        ).fetchone()
        if not tr or int(tr["is_active"]) != 1:
    ...
    emb = discord.Embed(
        ...
        description=f"Correct answer recorded.\nSlot holder: {winner.mention if winner else f'<@{winner_id}>'}\n\nHost: generate the arrange question with **/wib q_order**."
    )
...
async def order(interaction: discord.Interaction, a: str, b: str, c: str, d: str, e: str):
    ...
    # Show card picking + puzzle submit controls
    ...
    if isinstance(channel, discord.TextChannel):
        ...
        await channel.send(embed=emb)
        if turns > 0:
            card_view = CardPickView(bot, interaction.guild_id, interaction.channel_id, box_id)
            await channel.send(
            ...
            await channel.send(
                ...
                view=SubmitPuzzleView(bot, interaction.guild_id, interaction.channel_id, box_id)
            )
...
async def pass_show(interaction: discord.Interaction):
    ...
    class PassSelect(discord.ui.Select):
        ...
        async def callback(self, ix: discord.Interaction):
            ...
            con2 = db()
            try:
                # clear pending, move slot, end turns
                con2.execute(
            ...
            if isinstance(channel, discord.TextChannel):
                await channel.send(embed=discord.Embed(
                    title="Slot Passed",
                    description=f"Slot moved to <@{target_id}>."
                ))
            await ix.followup.send("Done.", ephemeral=True)

    view = discord.ui.View(timeout=300)
    view.add_item(PassSelect())
    await interaction.response.send_message(embed=discord.Embed(title="PASS Selection", description=f"Only <@{slot_user_id}> may select."), view=view)
...
async def steal_show(interaction: discord.Interaction):
    ...
    try:
        ...
        # Stealable: opened boxes 1-5 only (never box 6)
        rows = con.execute(
    ...
    class StealView(discord.ui.View):
        def __init__(self):
            ...
            for r in rows:
                ...
                label = f"Steal Box {bid}"
                self.add_item(StealButton(bid, owner))
    ...
    class StealButton(discord.ui.Button):
        ...
        async def callback(self, ix: discord.Interaction):
            ...
            if isinstance(channel, discord.TextChannel):
                ...
                await post_boxes_leaderboard(channel, ix.guild_id, ix.channel_id)
    ...
    for r in rows:
        bid = int(r["box_id"])
        owner = int(r["owner_user_id"])
        lines.append(f"Box {bid} — owner: <@{owner}>")
...
async def donate_show(interaction: discord.Interaction):
    ...
    try:
        ...
        # Boxes owned by slot holder (1-5 only)
        owned = con.execute(
...
async def check_puzzle(interaction: discord.Interaction):
    ...
    try:
        ...
        # If solved -> box ready
        if solved:
            # mark slot to None turns 0, clear pending
            con.execute(
    ...
    if solved:
        await interaction.response.send_message(embed=discord.Embed(
            ...
            description=f"Result: ✅ Correct.\nPhrase confirmed.\n\nMike may open the box with **/wib open_box**."
        ))
        return

    # Not solved: select next closest among remaining unchecked attempts
    con2 = db()
    try:
        secret2 = con2.execute(
            "SELECT phrase_w1, phrase_w2, phrase_w3 FROM box_secrets WHERE guild_id=? AND channel_id=? AND box_id=?",
            (interaction.guild_id, interaction.channel_id, box_id),
        ).fetchone()
        correct2 = (secret2["phrase_w1"], secret2["phrase_w2"], secret2["phrase_w3"])
        nxt = next_closest_puzzle_attempt(con2, interaction.guild_id, interaction.channel_id, box_id, correct2, int(attempt["attempt_id"]))
        if nxt:
            # Assign slot holder to nxt.user_id; clear turns/pending
            con2.execute(
                "UPDATE slot_state SET slot_user_id=?, turns_left=0, pending_action=NULL WHERE guild_id=? AND channel_id=? AND box_id=?",
    ...
    if nxt:
        ...
        await interaction.response.send_message(embed=discord.Embed(
            title=f"Box {box_id} — Puzzle Check",
            description=f"Result: ❌ Not solved.\nMatches in correct position: **{score}/3**.\nNo further pending attempts. Host may continue the round flow (e.g., run a new numeric question if desired)."
        ))
...
async def open_box(interaction: discord.Interaction):
    ...
    try:
        ...
        # Verify puzzle solved: require score=3 in any checked attempt OR at least words fully revealed (not enforced).
        secret = con.execute(
            "SELECT phrase_w1, phrase_w2, phrase_w3 FROM box_secrets WHERE guild_id=? AND channel_id=? AND box_id=?",
            (interaction.guild_id, interaction.channel_id, box_id),
        ).fetchone()
        if not secret:
            return await interaction.response.send_message("Box secret missing.", ephemeral=True)
        ...
        # Determine solver = user from that solved attempt (latest)
        solver = con.execute(
        ...
        if not prize or not (prize["title"] or "").strip():
            # prompt modal to fill now
            await interaction.response.send_modal(PrizeModal(interaction.guild_id, interaction.channel_id, box_id))
            return

        # Assign ownership of this box prize to solver (unless already set)
        con.execute(
        ...
        con.execute(
            ...
            (opened_boxes_count, eliminations_unlocked, next_box if next_box <= 6 else 6, interaction.guild_id, interaction.channel_id),
        )
    ...
    if isinstance(channel, discord.TextChannel):
        ...
        # Prepare next box secret if any
        if box_id < 6:
            con2 = db()
            try:
                sess2 = con2.execute("SELECT session_seed FROM sessions WHERE guild_id=? AND channel_id=?", (interaction.guild_id, interaction.channel_id)).fetchone()
...
async def elim_eligible(interaction: discord.Interaction):
    ...
    con = db()
    try:
        sess = con.execute("SELECT opened_boxes_count, eliminations_unlocked FROM sessions WHERE guild_id=? AND channel_id=?",
...
@wib.command(name="elim_choose", description="Host: eliminate one eligible player (0 boxes owned).")
async def elim_choose(interaction: discord.Interaction):
    if not isinstance(interaction.user, discord.Member) or not has_host_role(interaction.user):
        return await interaction.response.send_message("Host permission required.", ephemeral=True)
    ...
    if not eligible:
        return await interaction.response.send_message("No eligible players.", ephemeral=True)

    class ElimSelect(discord.ui.Select):
        def __init__(self):
            options = [discord.SelectOption(label=p["display_name"][:100], value=str(p["user_id"])) for p in eligible[:25]]
            super().__init__(placeholder="Select player to eliminate", min_values=1, max_values=1, options=options)
        ...
        async def callback(self, ix: discord.Interaction):
            ...
            try:
                con2.execute(
                    "UPDATE participants SET eliminated=1 WHERE guild_id=? AND channel_id=? AND user_id=?",
                    (ix.guild_id, ix.channel_id, target_id),
                )
                con2.commit()
            finally:
                con2.close()
            ...
            channel = ix.channel
            if isinstance(channel, discord.TextChannel):
                await channel.send(embed=discord.Embed(title="Eliminated", description=f"<@{target_id}> has been eliminated."))
            await ix.followup.send("Done.", ephemeral=True)

    view = discord.ui.View(timeout=300)
    view.add_item(ElimSelect())
    await interaction.response.send_message(embed=discord.Embed(title="Eliminate Player", description="Select one eligible player (0 boxes owned)."), view=view)
